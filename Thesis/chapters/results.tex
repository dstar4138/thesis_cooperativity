...
Despite giving it every opportunity to increase the spread, the catch of the parent
...
We will first consider the case of $ClusterComm_{(N,M)}$ with several values of
...
\todo[inline]{Compile notes and graphs of $ClusterComm_{(N,M)}$ and $PTree_{(W,N)}$.
    Show that batching aids in reducing channel block time by comparing it
    to $MTRRWS$ or $MTGQ$.
}
...
\todo[inline]{
    This section needs to compare reduction density and queue length of 
    several different tests. Namely, $ClusterComm_{(N,M)}$ where the number 
    of channels is equal to the number of processors. We should also
    include the $Interactivity_{(N,M)}$ tests here where $M=0$ since we only
    care about communication costs. This is similar to the analysis done in
    the following section. I refer you to that until this is complete.
}
...
We will first look at the two extremes for communication behaviour, completely
...
difference in execution time. \todo{Why did it run faster?}
...
%        & $MTRRWS$-$SQ$       & Shuffling Scheduler       \\ \cline{2-3} 
%\multicolumn{1}{c|}{\rotatebox{90}{\rlap{$Interactivity_{(20,0)}$}}} & 
%    \multicolumn{1}{c|}{\includegraphics[scale=0.32]{tests/interactivity/20/ss/ca/pg_0001.pdf}} & 
%    \multicolumn{1}{c|}{\includegraphics[scale=0.32]{tests/interactivity/20/wssq/ca/pg_0001.pdf}} \\ \cline{2-3} 
%\multicolumn{1}{c|}{\rotatebox{90}{\rlap{$ClusterComm_{(20,5)}$}}}   & 
%    \multicolumn{1}{c|}{\includegraphics[scale=0.32]{tests/clustercomm/20/ss/ca/pg_0001.pdf}} & 
%    \multicolumn{1}{c|}{\includegraphics[scale=0.32]{tests/clustercomm/20/wssq/ca/pg_0001.pdf}} \\ \cline{2-3} 
%\end{tabular}
%\caption{Comparison of Random and Uniform synchronization for $MTRRWS$-$SQ$ and 
%the Bipartite-Graph Aided Shuffling Scheduler using Absorption Channels.}
%    \label{tab:ss-compare-rand-uniform-ca}
%\end{table}
%
%

\todo[inline]{ Currently missing the duplicate of table 4.7 for absorption 
    channels. The tests are completed, it just is a matter of formating and
    transferring my notes on the topic.
}
...
speed up execution. \todo{\footnotesize I feel like this is missing something to tie it all up.}
