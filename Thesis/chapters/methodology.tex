...
To examine the effects of cooperativity-conscious schedulers we needed
...
source code. It would also be beneficial if our solution could visualize
these differences as with Haskell's ThreadScope \cite{jones2009parallel}.

Our solution, $ErLam$, is a compiler for an experimental version of the lambda
calculus with swap channels and a runtime system which allows for swappable
scheduler mechanisms and an optional logging system whose output can be fed
into a report generator. We have organized the details of our solution into
three sections.
Section~\ref{sec:erlam} 
will discuss the syntax and semantics of the ErLam language and demonstrate the
scheduler API by breaking down the CML Interactivity Scheduler.
Section~\ref{sec:simulation and visualization} will detail our testing
environment which consists of a logging system, a report generator, and a set
of example applications we used to demonstrate different levels of
cooperativity.  Finally, Section~\ref{sec:cooperativity mechanics} will detail
some example schedulers that demonstrate cooperativity-conscious behavior.
These schedulers will provide a basis for the results presented in
...
The ErLam Language is based on the lambda calculus, with first-class single 
variable functions, but deviates somewhat in that it provides other first-class 
entities. It deviates from Church representation to provide integers,
purely for ease of use. It also provides a symmetric synchronous channel type 
for inter-process communication. As a note, this language can also be classified 
...
Figure~\ref{fig:grammer} expresses ErLam's grammar in simplified BNF. The
semantics for the language are fairly straightforward. To summarize, each
expression reduces to one of the following terminal types: $integer$,
$channel$, or $function$. Each spawn passed an integer or a channel simply
evaluates to $0$ (\eg~false) without any further effect. Each spawn passed a
function, evaluates to $1$ (\eg~true) and applies $nil$ to that function within
a new ErLam process.  For a more detailed overview of ErLam's operational
semantics, see Appendix~\ref{app:semantics}.

ErLam provides several features for convenience such as a default branch
operator and a set of built-in functions including numeric operations, type
checking functions, and standard functional behaviors (\eg~combinators, \etc).
These built-in functions will be largely ignored throughout this document, but
explained when occasionally necessary. 

ErLam also extends this basic grammar with some useful syntactic sugar (see
Figure~\ref{fig:sugar-transform}) including SML-style $let$ expressions and
multi-variable function definitions which are curried from left to right. We
will use this syntactic sugar throughout this document to make our source
easier to review.
...
process to access the channel will be absorbed by it. The scheduler which 
evaluated the swap will be out a process and the scheduler which unblocks the channel 
by arriving second will get back two processes: the process performing the swap 
and the absorbed process. In terms of scheduling efficiency this type of message
