...
However, in the former's case, the channel has blocked and the only course of 
action for the scheduler is to get another expression to work on. In the 
later's case, we have an expression to work on, but we also may have unblocked 
other processes in the process so we need to reschedule them. 
...
time and work on something in the mean time. Thus, there a several possible
...
The first row shows the arbitrary timeslice $t_1$ where the process swapping $0$, 
$p0$, first contacts the channel with its value. The process my be scheduled again,
repeatedly, to check the channel up to some arbitrary timeslice $t_{n-1}$. At
...
when it returns. Thus on the third line, for any arbitrary timeslice in the 
...
defined which requires a minimal of $5$ callback functions 
...
\emph{layout/2} function with the NUMA layout of the system that the application is
...
Concurrent ML is an extension to SML which adds the \emph{spawn} function, and 
    ...
    \item {\bf The Multi-Threaded Round-Robin Work-Stealing Scheduler {\sl (MTRRWS)}} \\
        ...
        \begin{itemize}
            ...
            \item {\bf Interrupting-Steal {\sl (MTRRWS-IS)}} \\
                Simulates sending a theif-process over to another scheduler.
...
would also be adventagous to be as finely grained as possible and leave it up to 
...
exteneous data but still demonstrate various levels of cooperativity and phase 
...
cooperativity, on some level in previous scheduling systems, has been captured 
before. Through techniques keeping in mind Cache-Affinity, channel efficiency,
...
like to look at systems which recognise particular behaviours which affect this.
...
channel never needs to be migrated if they exist in the same batch and on the
...
from the batch it's apart of (unless it's a singleton). There are of course
alternatives to this (\eg~allow for $N$ quantum to pass before kicking a 
process from a batch), but we for testing purposes we can just extend the size
...
more on these assumptions in Chapter~\ref{chap:results}.
...
compared, cost of creating a channel and channel usage of the application it 
would be running with. In stark contrast to the previous scheduler, channel
...
The rational for this is, in the event of a block, the local LPU won't gain 
...
channel. A best case in this scenario would be a $PTree_W$ when $W$ is equal
...
Both of the previous schedulers ignore the concept ignore the effects ordering
...
as possible (\ie~the scheduler would not choose a process which could not unblock it).
...
processes need to be relocated. However, frequent sorting may cause the scheduler's
...
Our default sorting mechanism sorts based on recency of the communications over
...
and $C_i$ is the set of channels $P_i$ communicated with. The default sort, $\Delta$,
...
sorting are non-existent. As such, we should only be concerned with sorting 
